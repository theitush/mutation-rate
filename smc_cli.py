...
def smc_cli(input_path, output_dir):
    df = pd.read_table(input_path)
    logs_dir = os.path.join(output_dir, 'logs')
    os.makedirs(logs_dir, exist_ok=True)
    cmd_path = os.path.join(logs_dir, 'smc_cli.cmd')
    cmd = "PYTHONPATH={PYTHON_PATH}\n"
    cmd += f"{PYTHON_PATH} {MODELING_REPO}/modeling.py -i {input_path} -o {output_dir} " + "-l ${PBS_ARRAY_INDEX}"
    jnums = (0, len(df)-1)
    create_pbs_cmd_file(path=cmd_path, alias='smc_cli', output_logs_dir=logs_dir, jnums=jnums, cmd=cmd, queue=QUEUE)
...
if __name__ == '__main__':
    ...
    smc_cli(input_path=args.input_path, output_dir=args.output_dir)
